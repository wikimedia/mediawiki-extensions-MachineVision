 ...
 * @param {number} [config.maxSuggestions] The maximum number of suggestions to
 *      display in the auto-suggest
 ...
 */
function EntityAutocompleteInputWidget( config ) {
	config = config || {};
	...
	EntityAutocompleteInputWidget.parent.call( this, $.extend( {}, config, {
		...
		classes: [ 'wbmad-entity-input' ].concat( config.classes || [] )
	} ) );
...
EntityAutocompleteInputWidget.prototype.onLookupMenuChoose = function ( item ) {
	...
	this.emit( 'lookupMenuChoose', data );
};
...
EntityAutocompleteInputWidget.prototype.getData = function () {
	return new SuggestionData( this.getValue(), this.entityId );
};
...
EntityAutocompleteInputWidget.prototype.getLookupMenuOptionsFromData = function ( data ) {
	...
	for ( i = 0; i < data.length; i++ ) {
		...
		item.$element.find( '.wbmad-entity-input__autocomplete' ).on( 'mousedown', this.onMousedown.bind( this ) );
		items.push( item );
	}
 ...
 * @param {Object} e
 */
EntityAutocompleteInputWidget.prototype.onKeydown = function ( e ) {
